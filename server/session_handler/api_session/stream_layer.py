...
class __WorkerStreamLayer:
    ...
    def __handler_video(self):
        while True:
            ...
            try:
                request_frame = self.__requests_queue.get(block=False)
            except Empty:
                continue
            with self.__lock:
    ...
    def __init__(
        ...
        self.__requests_queue: Queue[RequestFrame] = Queue(session_settings.max_requests)
        self._current_offset = 0
        self.__lock = Lock()
        self._pause_lock = Lock()
        self._kill_event = Event()
        self.__worker_thread = Thread(target=self.__handler_video, args=(self,))
...
class StreamLayer(__WorkerStreamLayer):
    ...
    def add_request(self, request_frame: RequestFrame):
        self.__requests_queue.put(request_frame, block=True)
