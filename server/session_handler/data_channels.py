...
class TCPChannel(BaseChannel):
    ...
    def read_datas(self) -> list[tuple[str]]:
        ...
        datas = self._sock.recv(1024).decode("utf-8").split(" ")
...
class UDPChannel(BaseChannel):
    def __init__(self, client_address: str, client_port: int) -> None:
    ...
    def __serialize_bytes(cls, video_package: bytes, offset: int) -> bytes:
        return struct.pack(
            f"ii{VIDEO_BYTES_SIZE}s", offset, len(video_package), video_package
        )

    def send_data(self, data: bytes):
        for index in range(0, len(data), VIDEO_BYTES_SIZE):
            package = self.__serialize_bytes(
                data[index : index + VIDEO_BYTES_SIZE], index
            )
            self._sock.sendto(package, (self.__client_address, self.__client_port))
        eof_package = self.__serialize_bytes(b"-1", -1)
        self._sock.sendto(eof_package, (self.__client_address, self.__client_port))
